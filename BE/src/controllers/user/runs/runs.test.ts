...
describe("User runs CRUD calls", () => {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      const otherUserRun = await Runs.findOne({ name: "exotic name" });
      if (!otherUserRun) {
        throw new Error("Run not found");
      }
      otherUserRunId = otherUserRun.uuid;
  ...
  it("should fetch all user's runs", async () => {
  ...
  it("should return 404 for a run that wasn't created by a user", async () => {
    const res = await request(app)
      .get(`/user/run/${otherUserRunId}`)
      .set("Authorization", `Bearer ${token}`);
  ...
  it("shouldn't fetch run of another user with a specific id", async () => {
    const res = await request(app)
      .get("/user/run")
      .query({ id: otherUserRunId })
      .set("Authorization", `Bearer ${token}`);
  ...
  it("should delete a run that was created by a user", async () => {
    const res = await request(app)
      .delete(`/user/run/${runId}`)
      .set("Authorization", `Bearer ${token}`);
  ...
  it("should return 404 when a user tries to delete a run of another user", async () => {
    const res = await request(app)
      .delete(`/user/run/${otherUserRunId}`)
      .set("Authorization", `Bearer ${token}`);
